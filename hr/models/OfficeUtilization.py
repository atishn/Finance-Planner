...
class OfficeUtilization(object):
        ...
        try:
            ...
            for client in self.office.clients:
                    ...
                    for x in range(0,12):
                        if user_utilization.utilization[x] is not 0:
                            if not ufu.utilization[x]:
                                ...
                                ufu.utilization[x].append(UserFullUtilizationEntry(client,user_utilization.utilization[x]))
                ...
                for ghost_utilization in client_utilization.ghost_utilization:
                    ...
                    for x in range(0,12):
                        if ghost_utilization.utilization[x] is not 0:
                            if not ufu.utilization[x]:
                                ufu.utilization[x] = [UserFullUtilizationEntry(client,ghost_utilization.utilization[x])]
                            else:
                                ufu.utilization[x].append(UserFullUtilizationEntry(client,ghost_utilization.utilization[x]))
                ...
                for freelance_utilization in client_utilization.freelancer_utilization:
                    try:
                        ...
                        ufu = None
                    if ufu == None:
                        ufu = UserFullUtilization(freelance_utilization.name,freelance_utilization.role,freelance_utilization.id)
                    for x in range(0,12):
                        if freelance_utilization.utilization[x] is not 0:
                            if not ufu.utilization[x]:
                                ufu.utilization[x] = [UserFullUtilizationEntry(client,freelance_utilization.utilization[x])]
                            else:
                                ufu.utilization[x].append(UserFullUtilizationEntry(client,freelance_utilization.utilization[x]))
            ...
            for ghost_client in self.office.ghost_clients:
                ...
                for user_utilization in ghost_client_utilization.user_utilization:
                    ...
                    for x in range(0,12):
                        if user_utilization.utilization[x] is not 0:
                            ufu.utilization[x].append(UserFullUtilizationEntry(None,user_utilization.utilization[x],ghost_client))
                            if not uau.utilization[x] or uau.utilization[x] == 0:
                                uau.utilization[x] = user_utilization.utilization[x]
                            else:
                                uau.utilization[x] += user_utilization.utilization[x]
                ...
                for ghost_utilization in ghost_client_utilization.ghost_utilization:
                    ...
                    for x in range(0,12):
                        if ghost_utilization.utilization[x] is not 0:
                            if not ufu.utilization[x]:
                                ufu.utilization[x] = [UserFullUtilizationEntry(None,ghost_utilization.utilization[x],ghost_client)]
                            else:
                                ufu.utilization[x].append(UserFullUtilizationEntry(None,ghost_utilization.utilization[x],ghost_client))
                    self.ghosts[ghost_utilization.id] = ufu
            ...
            for freelancer in self.office.freelancers:
                ...
                freelancer_utilization = UserUtilization(freelancer.name,freelancer.role.name,freelancer.id,self.year,freelancer.start_date,freelancer.end_date,freelancer.utilization)
                if freelancer_utilization.is_active:
                    if old_freelancer_utilization is None:
                        self.office_freelancers[freelancer.id] = freelancer_utilization
                    else:
                        old_freelancer_utilization.addUtilization(freelancer_utilization)
                        self.office_freelancers[freelancer.id] = old_freelancer_utilization   
                    
            office_freelancer_utilization = self.office_freelancers.values()
            for freelance_utilization in office_freelancer_utilization:
                try:
                    ufu = self.freelancers[freelance_utilization.id]
                except:
                    ufu = None
                ...
                for x in range(0,12):
                    if freelance_utilization.utilization[x] is not 0:
                        if not ufu.utilization[x]:
                            ...
                            ufu.utilization[x].append(UserFullUtilizationEntry(None,freelance_utilization.utilization[x]))
                    self.freelancers[freelance_utilization.id] = ufu
