...
class Office(Base):
    ...
    def getTotalOverheadPerQuarter(self,year):
        ...
        total_overhead = [per_quarter_overhead,per_quarter_overhead,per_quarter_overhead,per_quarter_overhead,self.allocated_salary_expense + self.sga_expense] 
        for u in self.users:
            if u.end_date is None:
                u.end_date = datetime.datetime(int(year),12,31)
            if u.percent_billable < 100:
                quarterly_non_billable = quarterly_money(year,u.start_date,u.end_date,u.loaded_non_billable_salary_per_day)
                for x in range(0,4):
