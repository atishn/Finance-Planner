...
class GhostClientFinancials(object):
    ...
    def calculateFinancials(self):
        ...
        for user_allocation in self.ghost_client.team:
            salary_per_day = user_allocation.user.loaded_salary_per_day * (user_allocation.utilization / 100)  * self.usd_to_local

            user_salary = quarterly_money(self.year,user_allocation.start_date,user_allocation.end_date,salary_per_day,None,"ghost_salary")

            for x in range(0, 4):
                self.expense_salary[x] = self.expense_salary[x] + user_salary[x]
        ...
        for ghost_allocation in self.ghost_client.ghost_team:
            ...
            for x in range(0, 4):
                self.expense_ghost[x] = self.expense_ghost[x] + ghost_salary[x]
        
        for x in range(0,4):
            self.expense_salary[4] = self.expense_salary[4] + self.expense_salary[x]
            self.expense_ghost[4] = self.expense_ghost[4] + self.expense_ghost[x]
