...
def client_utilization(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),utilization=utilization,client=client,year=year,user=user,account=account,access_pipeline=access_pipeline,access_utilization=access_utilization,access_financials=access_financials)
    except:
        return HTTPFound(request.application_url)
...
def client_financials(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),financials=financials,client=client,year=year,user=user,account=account,access_pipeline=access_pipeline,access_utilization=access_utilization,access_financials=access_financials)
    except:
        ...
        if request.method=="POST":
            ...
            if client is None and office is not None and user.can_access_office(office,"financials"):
                ...
                if request.params.get("add_another") is None:
                    return HTTPFound(request.application_url + "/office/" + str(office_id) + "/clients/" + str(datetime.datetime.now().year))
...
def client_edit(request):
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if office_temp is None or user.can_access_client(client,"financials") == False or user.can_access_office(office_temp,"financials") == False:
                return HTTPFound(request.application_url)
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),offices=offices,user=user,client=client,account=account)
    except:
        return HTTPFound(request.application_url)
        
@view_config(route_name='client_assign_resource',request_method='POST',renderer='templates/client_assign_resource.html')
@view_config(route_name='client_assign_resource',request_method='GET',renderer='templates/client_assign_resource.html')
def client_assign_resource(request):
    try:
        ...
        if request.method=="POST":
            ...
            DBSession.add(ua)
            DBSession.flush()
            return HTTPFound(request.application_url + "/client/" + str(client_id) + "/utilization/" + str(datetime.datetime.now().year))
        ...
        if len(users) == 0:
            return HTTPFound(request.application_url)
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),clients=clients,users=users,user=user,account=account)
    except:
        print("*****")
        traceback.print_exc()
        return HTTPFound(request.application_url)
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if ghost_client_id is not None and len(ghost_client_id) > 0 and ghost_client_id != '':
                ghost_client = DBSession.query(GhostClient).filter_by(account_id=account_id).filter_by(id=long(ghost_client_id)).first()
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),clients=clients,ghost_users=ghost_users,user=user,account=account,ghost_clients=ghost_clients)
    except:
        print("*****")
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def client_delete(request):
    ...
    try:
        ...
        return HTTPFound(request.application_url + "/office/" + str(client.office_id) + "/clients/" + str(datetime.datetime.now().year)) 
    except:
        return HTTPFound(request.application_url)
