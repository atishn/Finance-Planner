...
def role_add(request):
    ...
    try:
        ...
        if request.method=="POST":
            ...
            department = DBSession.query(Department).filter_by(id=department_id).filter_by(account_id=account.id).first()
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("administration"),departments=departments,user=user)
    except:
        return HTTPFound(request.application_url)
...
def role_edit(request): 
   try:
       ...
       departments = DBSession.query(Department).filter_by(account_id=long(request.session['aid'])).all()
       return dict(logged_in = authenticated_userid(request),header = Header("administration"),departments=departments,user=user,role=role)
   except:
       return HTTPFound(request.application_url)
