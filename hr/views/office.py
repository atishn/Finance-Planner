...
def office_clients(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),financials=financials,office=office,year=year,user=user,account=account,access_pipeline=access_pipeline,access_utilization=access_utilization,access_financials=access_financials)
    except:
        return HTTPFound(request.application_url)
...
def office_utilization(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),utilization=utilization,office=office,year=year,user=user,account=account,access_pipeline=access_pipeline,access_utilization=access_utilization,access_financials=access_financials)
    except:
        return HTTPFound(request.application_url)
...
def office_pipeline(request):
    ...
    try:
        ...
        financials = office.getFinancials(year,user)
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),financials=financials,office=office,account=account,year=year,user=user,access_pipeline=access_pipeline,access_utilization=access_utilization,access_financials=access_financials)
    except:
        return HTTPFound(request.application_url)
...
def office_add(request):
    try:
        ...
        if request.method=="POST":
            ...
            office = DBSession.query(Office).filter_by(account_id=long(request.session['aid'])).filter_by(name=name).first()
            if office is None:
...
def office_edit(request):
    try:
        ...
        if request.method=="POST":
            name = request.params["name"].lower()
            off = DBSession.query(Office).filter_by(account_id=long(request.session['aid'])).filter_by(name=name).first()
