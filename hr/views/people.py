...
def people(request):
    ...
    if ((office_id is None or office_id.lower() == "all") and (department_id is None or department_id.lower() == "all")):
        if type_of_person == "direct":
            people = DBSession.query(User).filter(User.account_id == long(request.session['aid'])).filter(User.is_active == True).filter(User.manager_id == user.id).all()
        elif (user.permissions.is_administrator or (user.permissions.can_view_all_departments and user.permissions.can_view_all_offices)):
            people = DBSession.query(User).filter(User.account_id == long(request.session['aid'])).filter(User.is_active == True).all()
        else:
            return HTTPFound(location=request.application_url)
    elif office_id is None or office_id.lower() == "all":
        if type_of_person == "direct":
            people = DBSession.query(User).filter(User.department_id == long(department_id)).filter(User.is_active == True).filter(User.manager_id == user.id).all()
        elif user.permissions.is_administrator or user.permissions.can_view_all_departments:
            people = DBSession.query(User).filter(User.department_id == long(department_id)).filter(User.is_active == True).all()
        else:
            dept_match = False
        ...
        if type_of_person == "direct":
            people = DBSession.query(User).filter(User.office_id == long(office_id)).filter(User.is_active == True).filter(User.manager_id == user.id).all()
        elif user.permissions.is_administrator or user.permissions.can_view_all_offices:
            people = DBSession.query(User).filter(User.office_id == long(office_id)).filter(User.is_active == True).all()
        else:
            office_match = False
        ...
        if type_of_person == "direct":
            people = DBSession.query(User).filter(User.department_id == long(department_id)).filter(User.office_id == long(office_id)).filter(User.is_active == True).filter(User.manager_id == user.id).all()
        elif user.permissions.is_administrator or (user.permissions.can_view_all_offices and user.permissions.can_view_all_departments):
            people = DBSession.query(User).filter(User.department_id == long(department_id)).filter(User.office_id == long(office_id)).filter(User.is_active == True).all()
        else:
            office_match = False
            dept_match = False
            for office in user.permissions.view_offices:
    ...
    return dict(logged_in = authenticated_userid(request),account = account, user = user, people = people_dist, active_people = "active",active_roles = "", header = header,office_id = office_id,department_id = department_id,type_of_person=type_of_person)
