...
def person_delete(request):
    ...
    try:
        ...
        person_id = request.matchdict.get('person_id')
        DBSession.query(User).filter_by(id=person_id).filter_by(account_id=account_id).update({'is_active': False,'end_date':datetime.datetime.now()})
        DBSession.flush()
...
def person_add(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header(source),offices=offices,roles=roles,currencies=currencies,user=user,departments=departments)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if u is None or u.id == person_id:
                ...
                permissions_department_financials = request.params.getall("permissions_department_financials")
                for department_id in permissions_department_financials:
                    department = DBSession.query(Department).filter_by(account_id=account_id).filter_by(id=client_id).first()
                    if department is not None:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header(source),departments=departments,offices=offices,roles=roles,user=user,person=person,currencies=currencies)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def person_assign_add(request):
    try:
        ...
        if request.method=="POST":
            ...
            return HTTPFound(request.application_url + "/office/" + str(person.office_id) + "/utilization/" + str(datetime.datetime.now().year))
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),clients=clients,ghost_clients=ghost_clients,person=person,user=user,account=account)
    except:
        print("*****")
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def person_assign_edit(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),clients=clients,ghost_clients=ghost_clients,person=person,user=user,account=account,assignments=assignments,source_id=source_id,source_type=source_type,year=str(datetime.datetime.now().year))
    except:
        print("*****")
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def person_assign_delete(request):
    try:
        ...
        return HTTPFound(location=request.application_url + "/person/" + str(person_id) + "/assign/edit?source_type=" + source_type + "&source_id=" + str(source_id))
    except:
        print("*****")
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def person_enable_login(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("administration"),person=person,user=user,password=password)
    except:
        return HTTPFound(request.application_url)
...
def person_disable_login(request):
    ...
    try:
        ...
        if person is not None:
            password = None
            DBSession.query(User).filter_by(id=person_id).filter_by(account_id=account_id).update({'password':password})
            DBSession.flush()
