...
def skillset_entry_add(request):
    ...
    if user.permissions.is_administrator or person.manager_id == user.id:
        if is_midyear and (datetime.datetime.now() > account.midyear_review_end_date or datetime.datetime.now() < account.midyear_review_start):
            return {'ok':'False'}
        if is_midyear == False and (datetime.datetime.now() > account.annual_review_end_date or datetime.datetime.now() < account.annual_review_start):
            return {'ok':'False'}
...
def skillset_entry_update(request):
    ...
    if user.permissions.is_administrator or person.manager_id == user.id:
        if skillset_entry.is_midyear and (datetime.datetime.now() > account.midyear_review_end_date or datetime.datetime.now() < account.midyear_review_start):
            return {'ok':'False'}
        if skillset_entry.is_midyear == False and (datetime.datetime.now() > account.annual_review_end_date or datetime.datetime.now() < account.annual_review_start):
            return {'ok':'False'}
