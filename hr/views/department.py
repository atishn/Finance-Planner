...
@view_config(route_name='department_utilization',request_method='GET',renderer='templates/department_utilization.html')
def department_utilization(request):
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),utilization=utilization,department=department,year=year,user=user,account=account,access_utilization=access_utilization,access_financials=access_financials)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def department_financials(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),financials=financials,department=department,year=year,user=user,account=account,access_utilization=access_utilization,access_financials=access_financials)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if dept is None or dept.id == department_id:
                DBSession.query(Department).filter_by(id=department_id).filter_by(account_id=account_id).update({'name': name})
                DBSession.flush()
                return HTTPFound(location=request.application_url + "/administration/company")

        department = DBSession.query(Department).filter_by(id=department_id).first()
        return dict(logged_in = authenticated_userid(request),header = Header("administration"),department=department,user=user)
    except:
        return HTTPFound(request.application_url)
...
def department_delete(request):
    try:
        ...
        DBSession.query(Department).filter_by(id=department_id).filter_by(account_id=account_id).update({'is_active': False})
        DBSession.flush()
