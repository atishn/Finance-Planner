...
def global_financials(request):
    ...
    try:
        ...
        if user is None or account is None or (user.is_administrator == False and user.permissions_global_financials == False):
            return HTTPFound(request.application_url)
        
        financials = account.getFinancials(year,user)
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),financials=financials,year=year,account=account,user=user)
...
def global_utilization(request):
    ...
    try:
        ...
        if user is None or account is None or (user.is_administrator == False and user.permissions_global_utilization == False):
            return HTTPFound(request.application_url)
        
        utilization = account.getUtilization(year)
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),utilization=utilization,year=year,account=account,user=user)
    
    except:
        return HTTPFound(request.application_url)

@view_config(route_name='global_department_utilization',request_method='GET',renderer='templates/global_department_utilization.html')
...
def global_expenses(request):
    ...
    try:
        ...
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),offices=offices,year=year,user=user)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)