    ...
    try:
        ...
        if request.method=="POST":
            ...
            if request.params.get("add_another") is None:
                return HTTPFound(request.application_url + "/client/" + str(client_id) + "/projects/" + str(datetime.datetime.now().year))
        ...
        if len(clients) == 0:
            return HTTPFound(request.application_url)
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),clients=clients,user=user,account=account)
    except:
        return HTTPFound(request.application_url)

@view_config(route_name='office_projects_update',request_method='POST',renderer='templates/office_projects_update.html')
@view_config(route_name='office_projects_update',request_method='GET',renderer='templates/office_projects_update.html')
def office_projects_update(request):
    try:
        ...
        if request.method == "POST":
            ...
            DBSession.flush()
            return HTTPFound(request.application_url + "/office/" + str(office.id) + "/clients/" + str(datetime.datetime.now().year))
            
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),user=user,account=account,office=office)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)

@view_config(route_name='client_projects_update',request_method='POST',renderer='templates/client_projects_update.html')
@view_config(route_name='client_projects_update',request_method='GET',renderer='templates/client_projects_update.html')
def client_projects_update(request):
    try:
        ...
        if request.method == "POST":
            ...
            DBSession.flush()
            return HTTPFound(request.application_url + "/client/" + str(client.id) + "/projects/" + str(datetime.datetime.now().year))

        return dict(logged_in = authenticated_userid(request),header = Header("financials"),user=user,account=account,client=client)
    except:
        traceback.print_exc()
        return HTTPFound(request.application_url)
...
def project_edit(request):
    ...
    try:
        ...
        if len(clients) == 0:
            return HTTPFound(request.application_url)
        return dict(logged_in = authenticated_userid(request),header = Header("financials"),clients=clients,user=user,account=account,project=project)
    except:
        return HTTPFound(request.application_url)
...
def project_delete(request):
    ...
    try:
        ...
        return HTTPFound(location=request.application_url + "/client/" + str(project.client_id) + "/projects/" + str(datetime.datetime.now().year))
    except:
        print("*************")
        traceback.print_exc()
        return HTTPFound(request.application_url)
