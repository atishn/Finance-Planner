...
def revised_end_date(end_date):
    ...
    if end_date.day == 30 and (end_date.month == 4 or end_date.month == 6 or end_date.month == 9 or end_date.month == 11):
        return datetime.datetime(end_date.year,end_date.month + 1,1)
    if end_date.month == 2 and (end_date.year == 2012 or end_date.year == 2016 or end_date.year == 2020) and end_date.day == 29:
            ...
            for salary in user.salary_history:
                if ((latest_date is None or salary.start_date >= latest_date) and salary.start_date <= day_counter):
                    ...
                    if kind == "total":
                        ...
                        temp_salary = salary.salary * (1 + user.account.benefits_and_bonus/100) * (salary.percent_billable/100)
                    elif kind == "non-billable":
                        temp_salary = salary.salary * (1 + user.account.benefits_and_bonus/100) * ((100 - salary.percent_billable)/100)
